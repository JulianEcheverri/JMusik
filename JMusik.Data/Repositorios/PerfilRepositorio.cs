...
{
    public class PerfilRepositorio : IRepositorioGenerico<Perfil>
    {
        private readonly TiendaDbContext _contexto;
        private readonly ILogger<PerfilRepositorio> _logger;
        private DbSet<Perfil> _dbSet;

        public PerfilRepositorio(TiendaDbContext contexto, ILogger<PerfilRepositorio> logger)
        {
            _contexto = contexto;
            _logger = logger;
            _dbSet = _contexto.Set<Perfil>();
        }
        ...
        {
            Perfil entity = await _dbSet.SingleOrDefaultAsync(u => u.Id == id);
